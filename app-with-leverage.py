...
def get_leverage(exchange, symbol):
    """
    Fetch the leverage for a given symbol from the exchange.
    """
    try:
    ...
    for position in open_positions_main:
        try:
            ...
            if position_id and not get_replica_trade_id(position_id):
                ...
                leverage = get_leverage(bybit_main, position['symbol'])

                if leverage:
                    # Set the leverage for the replica account
                    market = bybit_replica.market(position['symbol'])
                    bybit_replica.user_post_leverage_save({
                        "symbol": market['id'],
                        "leverage": leverage
                    })
                ...
                print(f"Replicated position: {position['symbol']} {direction.upper()} {position_amount} with Leverage {leverage}")
            else:
