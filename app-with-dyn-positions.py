...
def fetch_balance():
    try:
        balance_data = bybit_replica.fetch_balance()
        total_balance = balance_data['total']['USDT']  # Assuming BTC as the currency
...
async def replicate_positions():
    ...
    if current_balance <= 0:
        print("Insufficient balance to open new positions.")
        return

    print(f"Current balance {current_balance}.")

    position_size_in_usdt = (current_balance / MAX_POSITIONS) * LEVERAGE
    ...
    for position in open_positions_main:
        try:
            ...
            if position_id and not get_replica_trade_id(position_id):
                ...
                ticker = bybit_replica.fetch_ticker(position['symbol'])
                current_price = ticker['last'] if 'last' in ticker else None
                if not current_price:
                    print(f"Unable to fetch current price for {position['symbol']}")
                    continue

                # Calculate the position amount in contracts/units
                position_amount = position_size_in_usdt / current_price
